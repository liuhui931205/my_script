...
krs = "http://192.168.7.22:13100/krs/"
url1 = krs + "image/get"
# url2 = krs + 'track/point/get?trackPointId=32914_20180804140801592680'
data = {
    "trackPointId": "32914_20180804140801592680",
    "type": "70",
    "seq": "001",
    "imageType": "png",
}
res_data = requests.post(url=url1, data=data)
# with open('./test_label.png', 'rb') as f:
#     olab_data = f.read()
# req_data = requests.get(url2)
# req_data = json.loads(req_data.text)
# trackid = req_data['result']['trackId']
lbimg_data = np.asarray(bytearray(res_data.content), dtype="uint8")
label_img = cv2.imdecode(lbimg_data, cv2.IMREAD_COLOR)
print(label_img.shape)
if label_img.shape[1] == 2448 and label_img.shape[0] == 2048:
    label_img = label_img[1024:2048, 0:2448]

for label in kd_road_labels:
    color = (label.color[2], label.color[1], label.color[0])
    s = np.where((label_img == color).all(axis=2), 1, 0)
    if np.all(s == 0):
        print("no")
    else:
        print('yes')
